...
//  Checklists
//
//  Created by Andela Developer on 6/11/15.
    ...
    func loadChecklists() {
        ...
        if NSFileManager.defaultManager().fileExistsAtPath(path) {
            if let data = NSData(contentsOfFile: path) {
                ...
                // there is no need to sort the list array here because, the list is sorted each time a checklist is added or editted, then the lists is saved in the Cheklist.plist file in that sorted order
            }
        }
    }
